...
const extensions: Problem[] = [
  ...
  },
  {
    ...
    concept: '論理演算',
    tags: ['論理演算'],
    statement: '西暦 y が与えられます。うるう年なら yes、そうでなければ no を出力してください。\nうるう年の条件: 4で割り切れ、かつ100で割り切れない。ただし400で割り切れる年はうるう年。\n\n入力例:\n2000\n\n出力例:\nyes',
    starterCode: 'y = int(input())\n',
    hints: ['and と or を組み合わせます。', '(y%4==0 and y%100!=0) or y%400==0。', '答え:\nleap = (y % 4 == 0 and y % 100 != 0) or y % 400 == 0\nprint("yes" if leap else "no")'],
    explanation: 'and(かつ)・or(または)・not(否定)で複数条件を組み立てる。\n\n模範解答:\ny = int(input())\nleap = (y % 4 == 0 and y % 100 != 0) or y % 400 == 0\nprint("yes" if leap else "no")',
    reference: 'y = int(input())\nleap = (y % 4 == 0 and y % 100 != 0) or y % 400 == 0\nprint("yes" if leap else "no")',
    testCases: [
    ...
    tip: '💡 and/or は「短絡評価」。a or b は a が真なら b を見ない。これを使った既定値設定も定番。',
  },
  {
    id: 'lv030-005',
    level: 30,
