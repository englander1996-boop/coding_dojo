...
const lv060 = build(60, 'lv060', [
  ...
  },
  {
    ...
    statement: '空白区切りの単語列。最も多く出た単語を1つ出力してください（同数なら初出順で先のもの）。\n\n入力例:\napple banana apple\n\n出力例:\napple',
    hints: ['Counter(...).most_common(1) は最頻1件のリスト。', '[0][0] で単語だけ取り出す。', '答え:\nfrom collections import Counter\nprint(Counter(input().split()).most_common(1)[0][0])'],
    explanation: 'most_common(1) は [(要素, 回数)] を返す。\n\n模範解答:\nfrom collections import Counter\nprint(Counter(input().split()).most_common(1)[0][0])',
    reference: 'from collections import Counter\nprint(Counter(input().split()).most_common(1)[0][0])',
    cases: [
      { input: 'apple banana apple\n', expected: 'apple', sample: true },
      { input: 'x\n', expected: 'x' },
      { input: 'a b a b a\n', expected: 'a' },
    ],
  },
