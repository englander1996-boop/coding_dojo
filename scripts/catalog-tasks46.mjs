...
t({ lv: 54, concept: '基礎: 最頻の文字', title: '最も多い文字(同点なら辞書順が小さい方)', tags: ['基礎', 'ハッシュ'], io: '英小文字の文字列が1行で与えられます。', ask: '最も多く現れる文字を出力してください。複数あれば辞書順で最小。', h1: '出現回数を数える。', h2: '(回数降順, 文字昇順)で選ぶ。', ref: 'from collections import Counter\ns=input()\ncnt=Counter(s)\nbest=None\nbestc=-1\nfor c in sorted(cnt):\n    if cnt[c]>bestc:\n        bestc=cnt[c]\n        best=c\nprint(best)', tip: '最頻文字の同点は辞書順で割る。先に sorted してから最大回数を探すと安定して最小が選べる。', ins: ['banana\n', 'abc\n', 'aabb\n', 'x\n', 'mississippi\n', 'zzzaaa\n'] })
